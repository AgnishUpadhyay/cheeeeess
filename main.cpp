...
#define Bqueen "\u2655"
#define Bking "\u2654"
#define initial "         a b c d e f g h\n         _______________\n       8|"
#define emptyLine "\n       | | | | | | | | |\n       "
#define endLine "\n         a b c d e f g h\n"
#define Nvalue 300
#define Bvalue 300
...
"3)En Passant move is not there ಥ﹏ಥ .\n"

"4)Pawn automatically promotes to Queen (No under promotions (´･_･`) )\n"

"5)Computer will take around 5 seconds (10 seconds max) to move. \n| Please Be Patient  ( ႎ _ ႎ ) |.\n"

"6)Also EndGame is Just terrible .·´¯`(>▂<)´¯`·.\n"
...
const int eg_pawn_table[64] =               {0,   0,   0,   0,   0,   0,   0,   0,
...
void initiateChessBoard(string Fen){
      ...
      for(int i=0; i<64; i++){
            switch(Board[i]){
                  case 'P': WP|=(1L<<i); whiteMaterial+=Pvalue; break;
                  case 'N': WN|=(1L<<i); whiteMaterial+=Nvalue; break;
                  case 'B': WB|=(1L<<i); whiteMaterial+=Bvalue; break;
                  case 'R': WR|=(1L<<i); whiteMaterial+=Rvalue; break;
                  case 'Q': WQ|=(1L<<i); whiteMaterial+=Qvalue; break;
                  case 'K': WK|=(1L<<i); whiteMaterial+=Kvalue; break;
                  case 'p': BP|=(1L<<i); blackMaterial+=Pvalue; break;
                  case 'n': BN|=(1L<<i); blackMaterial+=Nvalue; break;
                  case 'b': BB|=(1L<<i); blackMaterial+=Bvalue; break;
                  case 'r': BR|=(1L<<i); blackMaterial+=Rvalue; break;
                  case 'q': BQ|=(1L<<i); blackMaterial+=Qvalue; break;
                  case 'k': BK|=(1L<<i); blackMaterial+=Kvalue; break;
            }
      }
...
list <Move> WhitePawnMoves(int64_t WWP){
      ...
      int64_t possibility;
      int64_t PAWN_MOVES = (WWP>>7)&BLACK_PIECES&(~FILE_A)&checkMaskWhite;//capture left
      possibility = PAWN_MOVES&~(PAWN_MOVES-1);
      while (possibility!=0){
            int i = trailing_zeros(possibility);
            if((possibility&afterPinMask(-1L,possibility<<7,i+7))!=0){
      ...
      PAWN_MOVES = (WWP>>9)&BLACK_PIECES&(~FILE_H)&checkMaskWhite;//capture right
      possibility = PAWN_MOVES&~(PAWN_MOVES-1);
      while (possibility!=0){
         int i = trailing_zeros(possibility);
         if((possibility&afterPinMask(-1L,possibility<<9,i+9))!=0){
      ...
      PAWN_MOVES = (WWP>>8)&EMPTY&checkMaskWhite;//walk straight
      possibility = PAWN_MOVES&~(PAWN_MOVES-1);
      while (possibility!=0){
            int i = trailing_zeros(possibility);
            if((possibility&afterPinMask(-1L,possibility<<8,i+8))!=0){
      ...
      PAWN_MOVES = ((WWP>>16)&EMPTY&RANK_5)&(EMPTY>>8)&checkMaskWhite;//walk 2 steps straight
      possibility = PAWN_MOVES&~(PAWN_MOVES-1);
      while (possibility!=0){
            int i = trailing_zeros(possibility);
            if((possibility&afterPinMask(-1L,possibility<<16,i+16))!=0){
...
list <Move> BlackPawnMoves(int64_t BBP){
      ...
      list<Move> moveList;
      int64_t possibility;
      int64_t PAWN_MOVES = (BBP<<7)&WHITE_PIECES&(~FILE_H)&checkMaskBlack;//capture left
      possibility = PAWN_MOVES&~(PAWN_MOVES-1);
      while (possibility!=0){
            int i = trailing_zeros(possibility);
            if((possibility&afterPinMask(-1L,possibility>>7,i-7))!=0){
      ...
      PAWN_MOVES = (BBP<<9)&WHITE_PIECES&(~FILE_A)&checkMaskBlack;//capture right
      possibility = PAWN_MOVES&~(PAWN_MOVES-1);
      while (possibility!=0){
         int i = trailing_zeros(possibility);
         if((possibility&afterPinMask(-1L,possibility>>9,i-9))!=0){
      ...
      PAWN_MOVES = (BBP<<8)&EMPTY&checkMaskBlack;//walk straight
      possibility = PAWN_MOVES&~(PAWN_MOVES-1);
      while (possibility!=0){
            int i = trailing_zeros(possibility);
            if((possibility&afterPinMask(-1L,possibility>>8,i-8))!=0){
      ...
      PAWN_MOVES = ((BBP<<16)&EMPTY&RANK_4)&(EMPTY<<8)&checkMaskBlack;//walk 2 steps straight
      possibility = PAWN_MOVES&~(PAWN_MOVES-1);
      while (possibility!=0){
            int i = trailing_zeros(possibility);
            if((possibility&afterPinMask(-1L,possibility>>16,i-16))!=0){
...
list<Move> WhiteBishopMoves(int64_t WWB){
      ...
      while(BinPlace != 0){
            ...
            possibility = afterPinMask(possibility,BinPlace,index);
            possibility &=(~WHITE_PIECES);
            long square = possibility&~(possibility-1);
            while (square != 0){
                  int i = trailing_zeros(square);
                  currentMove.startIndex = index;
                  currentMove.targetIndex = i;
...
list<Move> WhiteKnightMoves(int64_t WWN){
      ...
      while (BinPlace != 0){
            int index = trailing_zeros(BinPlace);
            if (index >= 21){
                  possibility = KNIGHT_RANGE<<(index-21);
            }
            else{
                  possibility = KNIGHT_RANGE>>(21-index);
            }
            if (index%8 < 4){
...
list<Move> WhiteKingMoves(int64_t WWK){
      ...
      while (BinPlace != 0)
      {
            int index = trailing_zeros(BinPlace);
            if (index >= 9){
                  possibility = KING_RANGE<<(index-9);
            }
            else{
                  possibility = KING_RANGE>>(9-index);
            }
            if (index%8 < 4){
                  possibility &= ~(FILE_H|4629771061636907072L);
            }
...
int64_t generateQueenMask(int KI,int index, long Queen, long King){
      int64_t mask = 0L;
      if(KI>index){
...
int64_t generateBishopmask(int KI, int index, long Bishop, long King){
      int64_t mask = 0;
      if(KI>index){
...
int64_t generateRookMask(int KI, int index, long Rook, long King){
      int64_t mask = 0L;
      if(KI > index){
...
void generateWhiteAtackingSquares(int64_t WWP,int64_t WWK,int64_t WWB,int64_t WWN,int64_t WWQ,int64_t WWR,int64_t BBP,int64_t BBR,int64_t BBK,int64_t BBN,int64_t BBB,int64_t BBQ){
      ...
      OCCUPIED = ~EMPTY;
      long WHITE_PAWN_ATTACK = ((WWP>>7)&(~RANK_8)&(~FILE_A))|((WWP>>9)&(~RANK_8)&(~FILE_H));//capture left and right
      whiteAttackSquares = WHITE_PAWN_ATTACK;
      if(((BBK<<7)&WWP)!=0){
            checkMaskBlack = BBK<<7;
            whiteCheckers++;
      }
      if(((BBK<<9)&WWP)!=0){
            checkMaskBlack = BBK<<9;
            whiteCheckers++;
      }
      long BinPlace = 0L;
      long possibility;
...
void generateBlackAtackingSquares(int64_t WWP,int64_t WWK,int64_t WWB,int64_t WWN,int64_t WWQ,int64_t WWR,int64_t BBP,int64_t BBR,int64_t BBK,int64_t BBN,int64_t BBB,int64_t BBQ){
      ...
      OCCUPIED = ~EMPTY;
      long BLACK_PAWN_ATTACK = ((BBP<<7)&(~RANK_8)&(~FILE_H))|((BBP<<9)&(~RANK_8)&(~FILE_A));//capture left and right
      blackAttackSquares = BLACK_PAWN_ATTACK;
      if(((WWK>>7)&BBP)!=0){
            checkMaskWhite = WWK>>7;
            blackCheckers++;
      }
      if(((WWK>>9)&BBP)!=0){
            checkMaskWhite = WWK>>9;
            blackCheckers++;
      }
      long BinPlace = 0L;
      long possibility;
...
int endGameEvaluation(){
      if(whiteMaterial<250 || blackMaterial<250){
            ...
            int distanceBetweenKings = abs(blackKingX-whiteKingX)+abs(blackKingY-whiteKingY)*1.2;
...
void performMove(Move *move){
      ...
      int64_t bitStart = (1L<<start);
      int64_t bitEnd = (1L<<end);
      *(move->ptr)&=~(bitStart);
      *(move->ptr)|= (bitEnd);
      switch(Board[end]){
            case '0': break;
...
int64_t* performMoveInternal(Move *move){
      int64_t startIndex = (1L<<(move->startIndex));
      int64_t targetIndex = (1L<<(move->targetIndex));
      int64_t *ptrr=NULL;
...
void AImove(){
      ...
      if(Board[bestMove.startIndex]=='k'){
            canBshort = false;
            canBlong = false;
            if(bestMove.targetIndex==2){
                  Board[0] = '0';
                  Board[3] = 'r';
            }
            else if(bestMove.targetIndex==6){
                  Board[7] = '0';
                  Board[5] = 'r';
            }
      }
